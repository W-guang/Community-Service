...
async function actionNoticeDelete({ openid, data }) {
  const user = await getOrCreateUser(openid)
  requireRole(user, ['staff', 'admin'])
  const doc = await db.collection(COL.notices).doc(data._id).get()
  if (!doc.data) throw new Error('公告不存在')
  await db.collection(COL.notices).doc(data._id).remove()
  // 同时清理该公告的已读记录
  await db.collection(COL.noticeReads).where({ noticeId: data._id }).remove()
  return ok({ _id: data._id })
}

module.exports = {
  actionNoticeCreate, actionNoticeList, actionNoticeDetail,
  actionNoticeMarkRead, actionNoticeStats, actionNoticeDelete,
}
